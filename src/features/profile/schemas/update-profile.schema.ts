...
export const updateProfileSchema = z.object({
  ...
  firstName: z.string().trim().min(1, 'First name is required').max(80, 'First name must be at most 80 characters'),
  middleName: z.string().max(80, 'Middle name must be at most 80 characters'),
  lastName: z.string().trim().min(1, 'Last name is required').max(80, 'Last name must be at most 80 characters'),
